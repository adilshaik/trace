...
export const OutputChannel = vscode.window.createOutputChannel('trace');
OutputChannel.show();
...
export function convertReproValsToVerificationSteps(
  ...
  const reproConfig = vscode.workspace.getConfiguration('trace.repro');
  if (reproVals.buildCommand.trim() !== '') {
    ...
    const buildStep: VerificationStepArgs = {
      ...
      defaultLogTextFilters:
        'DEBUG-trace:|error|Error|exception|Exception|fail|Fail',
